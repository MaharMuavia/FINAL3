...
def plan_analysis(
    ...
    if dataset_type in {"ai_khata_transaction_report", "business_transaction_dataset"}:
        required_by_intent = {
            "revenue_trend": ["date", "transaction_type", "amount"],
            "sales_items": ["transaction_type", "item_customer", "amount"],
            "expense_summary": ["transaction_type", "item_customer", "amount"],
            "udhaar_summary": ["transaction_type", "item_customer", "amount"],
            "profit_summary": ["transaction_type", "amount"],
            "transaction_type_performance": ["transaction_type", "amount"],
        }
        required = required_by_intent.get(intent, [])
        missing = _missing(semantic, required)
        return AnalysisPlan(
            dataset_type=dataset_type,
            intent=intent,
            tool_name=f"ai_khata.{intent}",
            answerable=not missing,
            reason="Matched an AI Khata/business transaction analysis tool.",
            required_roles=required,
            missing_roles=missing,
            limit=limit,
            params=routed,
            clarification=(
                "I need Date, Category, Item/Customer, and Amount columns to run AI Khata transaction analytics."
