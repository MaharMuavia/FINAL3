...
from .ai_khata_tools import AIKhataTools
...
class DataAnalysisAgent:
    ...
    def answer(
        ...
        elif dataset_type in {"ai_khata_transaction_report", "business_transaction_dataset"}:
            result = self._answer_ai_khata(df, intent, limit, plan.params)
    ...
    def _answer_ai_khata(self, df: pd.DataFrame, intent: str, limit: int, routed: dict[str, Any]) -> dict[str, Any]:
        tools = AIKhataTools(df)
        if intent == "revenue_trend":
            return tools.revenue_trend(period=routed.get("period") or "M")
        if intent == "sales_items":
            return tools.sales_items(limit=limit)
        if intent == "expense_summary":
            return tools.expense_summary(limit=limit)
        if intent == "udhaar_summary":
            return tools.udhaar_summary(limit=limit)
        if intent == "profit_summary":
            return tools.profit_summary()
        if intent == "transaction_type_performance":
            return tools.transaction_type_performance(limit=limit)
        return tools.dataset_overview()
