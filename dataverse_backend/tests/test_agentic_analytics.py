...
def sample_ai_khata_df():
    ...
    df.attrs["report_metadata"] = {
        "report_title": "AI Khata Report",
        "Shop Name": "Muhammad Muavia",
        "Report Filter": "all",
        "Generated At": "2026-05-01 11:43:47",
    }
    df.attrs["report_summary"] = {
        "Total Sales": "7000",
        "Total Expenses": "500",
        "Udhaar Outstanding": "500",
        "Net Profit": "6500",
...
def test_ai_khata_revenue_by_month_uses_sales_rows_only():
    result = DataAnalysisAgent().answer(sample_ai_khata_df(), "Show revenue by month")

    assert result["dataset_type"] == "ai_khata_transaction_report"
    assert result["intent"] == "revenue_trend"
    assert result["business_summary"]["total_sales"] == 7000
    assert result["business_summary"]["total_expenses"] == 500
    assert result["business_summary"]["udhaar_outstanding"] == 500
    assert result["business_summary"]["net_profit"] == 6500
    assert result["tables"][0]["rows"] == [{"period": "2026-05", "sales_revenue": 7000}]
    assert "Only one period is available" in result["answer"]
    assert result["charts"][0]["y_key"] == "sales_revenue"


def test_ai_khata_category_performance_explains_transaction_type_not_product_category():
    result = DataAnalysisAgent().answer(sample_ai_khata_df(), "Which category performs best?")

    assert result["intent"] == "transaction_type_performance"
    assert result["tables"][0]["rows"][0] == {"transaction_type": "SALES", "amount": 7000}
    assert {"Item/Customer": "General Entry", "sales_revenue": 5000} in result["tables"][1]["rows"]
    assert {"Item/Customer": "burger", "sales_revenue": 2000} in result["tables"][1]["rows"]
    assert "Category is transaction type" in result["answer"]
    assert "no separate product category column" in result["answer"]
